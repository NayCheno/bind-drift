...
`kunit_case` produced `W-000178` and is included as a negative/failure-analysis case with adjudicated label `BENIGN_DRIFT`.
...
- Reviewer 1: `BENIGN_DRIFT` -- C-side drift and Rust exposure exist for kunit_case, but wrapper evidence is broad/not exact and semantic impact is not established.
